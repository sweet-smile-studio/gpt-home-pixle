...
function homeIcon(name, alt="", cls=""){
  return `<img class="home-ref-icon ${cls}" src="icons/home/${name}.jpg" alt="${escapeAttr(alt)}" draggable="false">`;
}
function homeCard(view,title,iconName,cls="",subtitle=""){
  return `<button class="home-card ${cls}" data-home-view="${view}" aria-label="${escapeAttr(title)}">
    ${homeIcon(iconName,title,"home-card-icon")}
    <span class="home-card-copy"><span class="home-card-title">${title}</span>${subtitle?`<span class="home-card-subtitle">${subtitle}</span>`:""}</span>
  </button>`;
}
...
function renderHome(){
  ...
  const moodFiles=["mood-energy","mood-focus","mood-recovery","mood-fatigue","mood-injury"];
  const activeMood=Math.max(0,Math.min(4,(Number(d.mood)||3)-1));
  const name=profile.name||"Leon";
  ...
  const avatar=profile.avatar || "icons/home/avatar.jpg";
  $('#home').innerHTML=`
    <div class="home-page home-reference-page">
      <header class="home-header">
        <div class="home-profile">
          <img src="${avatar}" alt="" class="home-avatar" draggable="false">
          <div class="home-greeting">
            <h1>Welcome back, ${esc(name)}</h1>
            <p>${dateText}</p>
          </div>
        </div>
        <button id="homeSettings" class="home-settings" aria-label="Open profile and settings"><img src="icons/home/gear.jpg" alt="" draggable="false"></button>
      </header>
        ...
        <div class="home-preview">
          <div class="preview-water home-glass-card">
            ...
            <div class="water-preview-control">
              <button data-water-minus class="home-round" aria-label="Subtract one liter">−</button>
              <button data-water-cup class="home-bottle" aria-label="Subtract one liter">${homeIcon("water-bottle","Water","home-bottle-image")}</button>
              <button data-water-plus class="home-round" aria-label="Add one liter">+</button>
            </div>
          ...
          <div class="preview-status home-glass-card">
            ...
            <div class="status-icons">
              ${moodLabels.map((label,i)=>`<button class="${i===activeMood?"active":""}" data-home-mood="${i+1}" title="${label}" aria-label="${label}">${homeIcon(moodFiles[i],label,"mood-image")}<small>${label}</small></button>`).join("")}
            </div>
            <button class="voice-preview" data-home-view="today" aria-label="Open today's voice notes">
          ...
          <button class="preview-note" data-home-view="today" aria-label="Open today's notes and gallery">
            ...
            ${homeIcon("pitch","Note/Entry","pitch-image")}
          </button>
        </div>
      </section>
      ...
      <section class="home-section">
        ...
        <div class="home-football-grid">
          ${homeCard("training","Training & Matches Calendar","calendar","training-large")}
          <div class="football-side">
            ${homeCard("performance","Performance & Stats","performance","wide")}
            <div class="football-two">
              ${homeCard("goals","Goals & Missions","goals")}
              ${homeCard("achievements","Achievements","achievements")}
            </div>
          </div>
        </div>
      </section>
      ...
      <section class="home-section">
        <h2>MIND & PLANNING</h2>
        <div class="home-four">
          ${homeCard("tactics","Notes & Tactics","tactics")}
          ${homeCard("ideas","Ideas","ideas")}
          ${homeCard("brain","Brain Dump","brain")}
          ${homeCard("tools","Tools","tools")}
        </div>
      </section>
      ...
      <section class="home-bottom">
        <div class="home-section grow">
          ...
          <div class="home-body-grid">
            ${homeCard("health","Diet & Health","diet","body-card","Today's data")}
            ${homeCard("habits","Habit Tracker","habits","body-card","Today's data")}
          </div>
        </div>
        <div class="home-section profile-card-section">
          <h2>PROFILE</h2>
          ${homeCard("profile","Profile & Settings","profile","body-card")}
        </div>
      </section>
    </div>`;
  bindHomeLinks();
  ...
  $$('#home [data-home-mood]').forEach(btn=>btn.addEventListener('click',()=>{
    d.mood=btn.dataset.homeMood; d.energy=Number(btn.dataset.homeMood); save(); renderHome();
  }));
}
...
function renderTraining(){
  const events=sortedFootballEvents();
  const training=(state.training||[]).slice().sort((a,b)=>`${a.date||""} ${a.time||""}`.localeCompare(`${b.date||""} ${b.time||""}`));
  const matches=(state.matches||[]).slice().sort((a,b)=>`${a.date||""} ${a.time||""}`.localeCompare(`${b.date||""} ${b.time||""}`));
  $("#training").innerHTML=`
    ${card("Training & Matches Calendar",`
      <div class="segmented">
        <button class="seg active" data-training-tab="all">همه</button>
        <button class="seg" data-training-tab="training">تمرینات</button>
        <button class="seg" data-training-tab="matches">مسابقات</button>
      </div>
      <div class="grid two football-form">
        <form id="trainingForm" class="panel panel-titleless">
          <div class="panel-body">
            <h3>افزودن تمرین</h3>
            <div class="form-grid">
              <input name="date" type="date" value="${iso(selected)}" required>
              <input name="time" type="time" value="18:00" required>
              <input name="title" placeholder="نوع تمرین" value="Training" required>
              <input name="focus" placeholder="تمرکز تمرین">
              <input name="location" placeholder="محل">
            </div>
            <textarea name="notes" placeholder="یادداشت تمرین"></textarea>
            <button class="premium-btn" type="submit">+ ثبت تمرین</button>
          </div>
        </form>
        <form id="matchForm" class="panel panel-titleless">
          <div class="panel-body">
            <h3>افزودن مسابقه</h3>
            <div class="form-grid">
              <input name="date" type="date" value="${iso(selected)}" required>
              <input name="time" type="time" value="19:00" required>
              <input name="opponent" placeholder="حریف" required>
              <input name="location" placeholder="محل">
              <input name="position" placeholder="پست">
            </div>
            <button class="premium-btn" type="submit">+ ثبت مسابقه</button>
          </div>
        </form>
      </div>
      <div id="trainingEventList" class="list">
        ${events.length ? events.map(x=>`
          <div class="list-item football-event-item">
            <div>
              <strong>${x.kind==="match"?"⚽ مسابقه":"🏃 تمرین"} — ${esc(x.kind==="match" ? ("vs "+(x.opponent||"حریف")) : (x.title||"تمرین"))}</strong>
              <div class="subtle">${esc(x.date||"")} · ${esc(x.time||"")} ${x.location?"· "+esc(x.location):""}</div>
              ${x.focus?`<div class="mini">Focus: ${esc(x.focus)}</div>`:""}
              ${x.notes?`<div class="mini">${esc(x.notes)}</div>`:""}
            </div>
            <button class="small-btn" data-football-delete="${escAttr(x.kind)}:${escAttr(x.id)}">حذف</button>
          </div>`).join("") : `<div class="empty-state">هنوز رویداد فوتبالی ثبت نشده است.</div>`}
      </div>
    `)}
  `;
  $$("#training [data-training-tab]").forEach(btn=>btn.addEventListener("click",()=>{
    $$("#training [data-training-tab]").forEach(x=>x.classList.remove("active"));
    btn.classList.add("active");
    const kind=btn.dataset.trainingTab;
    $$("#training [data-football-delete]").forEach(x=>{
      const [k]=x.dataset.footballDelete.split(":");
      x.closest(".list-item").style.display=(kind==="all"||kind===k)?"":"none";
    });
  }));
  $("#trainingForm")?.addEventListener("submit",e=>{
    e.preventDefault();
    const f=new FormData(e.currentTarget);
    state.training.push({id:uid(),date:f.get("date"),time:f.get("time"),title:f.get("title"),focus:f.get("focus"),location:f.get("location"),notes:f.get("notes")});
    save(); renderTraining();
  });
  $("#matchForm")?.addEventListener("submit",e=>{
    e.preventDefault();
    const f=new FormData(e.currentTarget);
    state.matches.push({id:uid(),date:f.get("date"),time:f.get("time"),opponent:f.get("opponent"),location:f.get("location"),position:f.get("position")});
    save(); renderTraining();
  });
  $$("#training [data-football-delete]").forEach(btn=>btn.addEventListener("click",()=>{
    const [kind,id]=btn.dataset.footballDelete.split(":");
    if(kind==="match") state.matches=state.matches.filter(x=>x.id!==id);
    else state.training=state.training.filter(x=>x.id!==id);
    save(); renderTraining();
  }));
}

function renderGoals(){
  const goals=state.goals||[], missions=state.missions||[];
  $("#goals").innerHTML=`
    ${card("Goals & Missions",`
      <form id="goalForm" class="football-form">
        <div class="form-grid">
          <input name="title" placeholder="هدف جدید، مثلا بهبود پای چپ" required>
          <input name="deadline" type="date" value="${iso(selected)}">
        </div>
        <textarea name="description" placeholder="توضیح هدف"></textarea>
        <button class="premium-btn" type="submit">+ افزودن هدف</button>
      </form>
      <div class="grid two">
        <div>
          <h3>اهداف</h3>
          <div class="list">${goals.length?goals.map(g=>`
            <article class="goal-card">
              <div>
                ...
                <label class="mini">Progress ${Number(g.progress)||0}%</label>
                <input class="goal-range" type="range" min="0" max="100" value="${Number(g.progress)||0}" data-goal-progress="${escAttr(g.id)}">
              </div>
              <button class="small-btn" data-goal-delete="${escAttr(g.id)}">حذف</button>
            </article>`).join(""):`<div class="empty-state">هنوز هدف ثبت نشده است.</div>`}</div>
        </div>
        <div>
          <h3>Missionها</h3>
          <form id="missionForm">
            <div class="form-grid">
              <input name="title" placeholder="ماموریت" required>
              <select name="goalId"><option value="">بدون هدف</option>${goals.map(g=>`<option value="${escAttr(g.id)}">${esc(g.title)}</option>`).join("")}</select>
            </div>
            <button class="premium-btn" type="submit">+ افزودن Mission</button>
          </form>
          <div class="list">${missions.length?missions.map(m=>`
            <div class="list-item">
              <label><input type="checkbox" data-mission-toggle="${escAttr(m.id)}" ${m.done?"checked":""}> ${esc(m.title)}</label>
              <button class="small-btn" data-mission-delete="${escAttr(m.id)}">حذف</button>
            </div>`).join(""):`<div class="empty-state">هنوز Mission ثبت نشده است.</div>`}</div>
        </div>
      </div>
    `)}
  `;
  $("#goalForm")?.addEventListener("submit",e=>{
    ...
    state.goals.push({id:uid(),title:f.get("title"),description:f.get("description"),deadline:f.get("deadline"),progress:0});
    save(); renderGoals();
  });
  $("#missionForm")?.addEventListener("submit",e=>{
    e.preventDefault(); const f=new FormData(e.currentTarget);
    state.missions.push({id:uid(),title:f.get("title"),goalId:f.get("goalId"),done:false,date:iso(selected)});
    save(); renderGoals();
  });
  $$("#goals [data-goal-progress]").forEach(r=>r.addEventListener("input",()=>{
    const g=state.goals.find(x=>x.id===r.dataset.goalProgress); if(g){g.progress=Number(r.value);save();}
  }));
  $$("#goals [data-goal-delete]").forEach(b=>b.addEventListener("click",()=>{state.goals=state.goals.filter(x=>x.id!==b.dataset.goalDelete);save();renderGoals();}));
  $$("#goals [data-mission-toggle]").forEach(b=>b.addEventListener("change",()=>{const m=state.missions.find(x=>x.id===b.dataset.missionToggle);if(m){m.done=b.checked;save();renderGoals();}}));
  $$("#goals [data-mission-delete]").forEach(b=>b.addEventListener("click",()=>{state.missions=state.missions.filter(x=>x.id!==b.dataset.missionDelete);save();renderGoals();}));
}

function renderPerformance(){
  const goals=state.goals||[], missions=state.missions||[];
  const avg=goalProgress(), done=missions.filter(x=>x.done).length;
  const total=missions.length;
  $("#performance").innerHTML=`
    ${card("",`
      <div class="performance-hero">
        <div><span class="eyebrow">YOUR FOOTBALL PROGRESS</span><h2>Performance & Stats</h2><p class="subtle">تحلیل پیشرفت شخصی بر اساس اهداف و مأموریت‌ها</p></div>
        <div class="performance-score">${avg}<small>/100</small></div>
      </div>
      <div class="grid three">
        <div class="stat-card"><span class="subtle">Goal progress</span><b>${avg}%</b></div>
        <div class="stat-card"><span class="subtle">Mission completion</span><b>${total?pct(done,total):0}%</b></div>
        <div class="stat-card"><span class="subtle">Active goals</span><b>${goals.filter(g=>Number(g.progress||0)<100).length}</b></div>
      </div>
    `)}
    ${card("Goal Analysis", goals.length?`<div class="list">${goals.map(g=>`<div class="list-item"><div><strong>${esc(g.title)}</strong><div class="subtle">${Number(g.progress)||0}% complete</div></div><div class="progress-mini"><span style="width:${Number(g.progress)||0}%"></span></div></div>`).join("")}</div>`:`<div class="empty-state">برای دیدن تحلیل، ابتدا Goal بساز.</div>`)}
  `;
}
...
function renderHealth(){
  ...
  const mealCount=(state.meals||[]).filter(m=>m.date===iso(selected)).length;
  $("#health").innerHTML=`
    ${card("Diet & Health",`
      <div class="grid three">
        <div class="health-metric"><span>Hydration</span><b>${Number(d.water)||0} L</b><small>Today</small></div>
        <div class="health-metric"><span>Mood / Energy</span><b>${Number(d.energy)||0}/5</b><small>Today</small></div>
        <div class="health-metric"><span>Meals logged</span><b>${mealCount}</b><small>Today</small></div>
      </div>
      <form id="healthForm" class="form-grid" style="margin-top:18px">
        <input name="sleep" type="number" min="0" step=".1" placeholder="Sleep hours" value="${escAttr(h.sleep||"")}">
        <input name="energy" type="number" min="0" max="5" placeholder="Energy 0-5" value="${escAttr(h.energy||"")}">
        <input name="recovery" type="number" min="0" max="5" placeholder="Recovery 0-5" value="${escAttr(h.recovery||"")}">
        <input name="weight" type="number" step=".1" placeholder="Weight" value="${escAttr(h.weight||"")}">
        <textarea name="notes" placeholder="Health notes">${esc(h.notes||"")}</textarea>
        <button class="premium-btn" type="submit">ذخیره سلامت</button>
      </form>
    `)}
    ${card("Meals",`
      <form id="mealForm" class="form-grid">
        ...
        <input name="notes" placeholder="Nutrition / notes">
        <button class="premium-btn" type="submit">+ ثبت وعده</button>
      </form>
      <div class="list">${mealCount?state.meals.filter(m=>m.date===iso(selected)).map(m=>`<div class="list-item"><span>${esc(m.time)} — ${esc(m.name)} ${m.notes?"· "+esc(m.notes):""}</span><button class="small-btn" data-meal-delete="${escAttr(m.id)}">حذف</button></div>`).join(""):`<div class="empty-state">وعده‌ای برای امروز ثبت نشده است.</div>`}</div>
    `)}
  `;
  $("#healthForm")?.addEventListener("submit",e=>{e.preventDefault();const f=new FormData(e.currentTarget);state.health={...state.health,sleep:f.get("sleep"),energy:f.get("energy"),recovery:f.get("recovery"),weight:f.get("weight"),notes:f.get("notes")};save();renderHealth();});
  $("#mealForm")?.addEventListener("submit",e=>{e.preventDefault();const f=new FormData(e.currentTarget);state.meals.push({id:uid(),date:iso(selected),name:f.get("name"),time:f.get("time"),notes:f.get("notes")});save();renderHealth();});
  $$("#health [data-meal-delete]").forEach(b=>b.addEventListener("click",()=>{state.meals=state.meals.filter(m=>m.id!==b.dataset.mealDelete);save();renderHealth();}));
}

function renderTactics(){
  const items=state.tactics||[];
  $("#tactics").innerHTML=card("Notes & Tactics",`
    <form id="tacticForm" class="football-form">
      <div class="form-grid"><input name="title" placeholder="عنوان تاکتیک / یادداشت" required><select name="category"><option>Tactics</option><option>Formation</option><option>Opponent</option><option>Coach Feedback</option><option>Training</option></select></div>
      <textarea name="body" placeholder="یادداشت تاکتیکی..." required></textarea>
      <button class="premium-btn" type="submit">+ افزودن یادداشت</button>
    </form>
    <div class="grid three">${items.length?items.map(x=>`<article class="tactic-card"><span class="eyebrow">${esc(x.category||"Tactics")}</span><h3>${esc(x.title)}</h3><p>${esc(x.body)}</p><button class="small-btn" data-tactic-delete="${escAttr(x.id)}">حذف</button></article>`).join(""):`<div class="empty-state">هنوز یادداشت تاکتیکی ثبت نشده است.</div>`}</div>
  `);
  $("#tacticForm")?.addEventListener("submit",e=>{e.preventDefault();const f=new FormData(e.currentTarget);state.tactics.push({id:uid(),title:f.get("title"),category:f.get("category"),body:f.get("body"),date:iso(selected)});save();renderTactics();});
  $$("#tactics [data-tactic-delete]").forEach(b=>b.addEventListener("click",()=>{state.tactics=state.tactics.filter(x=>x.id!==b.dataset.tacticDelete);save();renderTactics();}));
}

function renderAchievements(){
  const items=state.achievements||[];
  $("#achievements").innerHTML=card("Achievements",`
    <form id="achievementForm" class="form-grid football-form">
      <input name="title" placeholder="نام افتخار" required>
      <input name="date" type="date" value="${iso(selected)}">
      <input name="icon" placeholder="🏆" value="🏆">
      <textarea name="description" placeholder="توضیح"></textarea>
      <button class="premium-btn" type="submit">+ افزودن افتخار</button>
    </form>
    <div class="achievement-grid">${items.length?items.map(x=>`<article class="achievement-card"><span>${esc(x.icon||"🏆")}</span><h3>${esc(x.title)}</h3><p>${esc(x.description||"")}</p><small>${esc(x.date||"")}</small><br><button class="small-btn" data-achievement-delete="${escAttr(x.id)}">حذف</button></article>`).join(""):`<div class="empty-state">هنوز افتخاری ثبت نشده است.</div>`}</div>
  `);
  $("#achievementForm")?.addEventListener("submit",e=>{e.preventDefault();const f=new FormData(e.currentTarget);state.achievements.push({id:uid(),title:f.get("title"),date:f.get("date"),icon:f.get("icon"),description:f.get("description")});save();renderAchievements();});
  $$("#achievements [data-achievement-delete]").forEach(b=>b.addEventListener("click",()=>{state.achievements=state.achievements.filter(x=>x.id!==b.dataset.achievementDelete);save();renderAchievements();}));
}
